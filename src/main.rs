...
fn build_ui(application: &gtk::Application) {
    ...
    listbox.bind_model(Some(&model),
        clone!(@weak window => @default-panic, move |item| {
            ...
            let spin_button = gtk::SpinButton::new_with_range(0.0, 1000000.0, 1.0);
            item.bind_property("count", &spin_button, "value")
                ...
                .build();
						...
						fbox.insert(&spin_button, -1);

            let entry_desc = gtk::Entry::new();
            item.bind_property("desc", &entry_desc, "text")
            		.flags(glib::BindingFlags::DEFAULT | glib::BindingFlags::SYNC_CREATE | glib::BindingFlags::BIDIRECTIONAL)
            		.build();
            fbox.insert(&entry_desc, -1);
        ...
        edit_button.connect_clicked(clone!(@weak window, @strong item => move |_| {
            ...
            let spin_button = gtk::SpinButton::new_with_range(0.0, 100000.0, 1.0);
            item.bind_property("count", &spin_button, "value")
                .flags(glib::BindingFlags::DEFAULT | glib::BindingFlags::SYNC_CREATE | glib::BindingFlags::BIDIRECTIONAL)
                .build();
            content_area.add(&spin_button);

            let entry_desc2 = gtk::Entry::new();
            item.bind_property("desc", &entry_desc2, "text")
            		.flags(glib::BindingFlags::DEFAULT | glib::BindingFlags::SYNC_CREATE | glib::BindingFlags::BIDIRECTIONAL)
            		.build();

            entry_desc2.connect_activate(clone!(@weak dialog => move |_| {
            	dialog.response(ResponseType::Close);
            }));

            content_area.add(&entry_desc2);

        //     let button = gtk::Button::new_with_label("Teste");
        //     button.connect_clicked(clone!(@weak dialog => move |_|{
								// dialog.response(ResponseType::Close);
        //     }));
            ...
            dialog.connect_response(clone!(@weak model, @weak entry, @weak spin_button => move |dialog, resp| {
                if let Some(text) = entry.get_text() {
                    if !text.is_empty() && resp == ResponseType::Ok {
                        model.append(&RowData::new(&text, spin_button.get_value() as u32, &entry_desc.get_text().unwrap().as_str()));
                    }
                }
                dialog.close();
            }));
    ...
    hbox.add(&delete_button);
		...
		// >>>>>> Continuar aqui <<<<<<<<
    // Adicionar a capacidade de salvar o conteúdo do liststore no disco.
    // let save_button = gtk::Button::new_with_label("Save");

    // save_button.connect_clicked(clone!(@weak model, @weak listbox => move |_|{
    ...
    // for i in 0..10 {
    //     model.append(&RowData::new(&format!("LINHA {}", i), i * 4, &format!("Descrição automática número: {}", i * 2)));
    // }
...
mod row_data {
    ...
    impl RowData {
        pub fn new(name: &str, count: u32, desc: &str) -> RowData {
            glib::Object::new(Self::static_type(), &[("name", &name), ("count", &count), ("desc", &desc)])
